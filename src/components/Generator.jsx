    ...
    title={["it's", "huge", "o\'clock"]}>
        ...
        {Object.keys(WORKOUTS).map((type, typeIndex) => {
            return (
                <button onClick={() => {
                    setMuscles([])
                    setPoison(type)
                }} className={' px-4 bg-slate-950 border  duration-200 px-4 hover:border-blue-600 py-3 rounded-lg ' + (type === poison ? ' border-blue-600' : ' border-blue-400')} key={typeIndex}>
        ...
        <div className='bg-slate-950  border border-solid border-blue-400 rounded-lg flex flex-col'>
            <button onClick={toggleModal} className='relative flex items-center justify-center p-3'>
                <p className='captalize'>{muscles.length == 0 ? 'select muscle groups' : muscles.join(' ')}</p>
        ...
        {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
            return (
                <button onClick={() => {
                    setGoal(scheme)
                }} className={' px-4 bg-slate-950 border  duration-200 px-4 hover:border-blue-600 py-3 rounded-lg ' + (scheme === goal ? ' border-blue-600' : ' border-blue-400')} key={schemeIndex}>
